...
func _ready():
	...
	for sfx_name in AudioManager.SFX_CONFIG.keys():
		...
		play_btn.text = " Play "
		play_btn.pressed.connect(func(): AudioManager.play_2d_sfx(sfx_name))
		...
		vol_val.text = "%5.1f dB" % start_vol
		vol_val.custom_minimum_size = Vector2(70, 0)
		...
		pitch_val.text = "1.00x"
		pitch_val.custom_minimum_size = Vector2(50, 0)
		...
		if AudioManager.SFX_CONFIG[sfx_name].has("max_distance"):
			...
			var max_dist_val = Label.new()
			var start_dist = AudioManager.SFX_CONFIG[sfx_name].get("max_distance", 30.0)
			max_dist_val.text = "%3.0f m" % start_dist
			max_dist_val.custom_minimum_size = Vector2(50, 0)
	...
	for music_name in AudioManager.MUSIC_CONFIG.keys():
		...
		var vol_val = Label.new()
		var start_vol = AudioManager.MUSIC_CONFIG[music_name].get("base_volume", 0.0)
		vol_val.text = "%5.1f dB" % start_vol
		vol_val.custom_minimum_size = Vector2(70, 0)
		row.add_child(vol_val)
